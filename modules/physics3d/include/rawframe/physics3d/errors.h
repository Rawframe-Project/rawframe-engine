#pragma once

#include "rawframe/base/bits128.h"
#include "rawframe/result/error.h"

#include <cstdint>

namespace rawframe::physics3d {

/// The domain of every Error this module creates.
inline constexpr result::ErrorDomain kPhysics3DDomain{base::parseBits128Hex("ec242ebe3c6aa68692cf9849ede85483").value};

/// Codes within kPhysics3DDomain.
enum class Physics3DError : std::uint32_t {
    /// This processor cannot run the physics build's kernels.
    Unsupported = 1,
    /// Settings out of range, or a World whose physics components are not
    /// the engine's.
    InvalidSettings = 2,
    /// No more physics worlds in this process, or a full world.
    Capacity = 3,
};

[[nodiscard]] constexpr result::ErrorCode code(Physics3DError error) noexcept {
    return result::ErrorCode{static_cast<std::uint32_t>(error)};
}

} // namespace rawframe::physics3d
