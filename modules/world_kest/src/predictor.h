...
// the same program the server runs. A game with physics also steps the
// player's body there, among the level's static bodies (D36).
...
struct PredictorSettings {
    ...
    /// The game's physics, for a game with it: the player's body is stepped
    /// among `level`, each entity's components and their values.
