...
# _CL_ after the command line, so -WX- there outlasts MsQuic's /WX, and -GL-
# its whole-program optimization, whose objects only MSVC's own linker reads.
# Written with a dash, since Git's bash turns /WX- into a path for Windows
# programs.
(
    if [ "$windows" = 1 ]; then
        export _CL_="-WX- -GL-"
